...
When `Decompress source records from Amazon CloudWatch Logs` is set to `On` data is not compressed
...
4) Concatenate the result from (4) together and set the result as the data of the record returned to Firehose. Note that
   this step will not add any delimiters. Delimiters should be appended by the logic within the transformLogEvent
   method.
5) Any individual record exceeding 6,000,000 bytes in size after decompression, processing and base64-encoding is marked
   ...
   round.
6) When the total data size (i.e. the sum over multiple records) after decompression, processing and base64-encoding
    ...
    for r in records:
        ...
        if data["messageType"] == "CONTROL_MESSAGE":
            ...
            joinedData = "".join(
                [
                    transformLogEvent(
                        e, data["owner"], arn, data["logGroup"], data["logStream"], data["subscriptionFilters"][0]
                    )
...
def lambda_handler(event, context):
    ...
    for idx, rec in enumerate(records):
        ...
        if len(rec["data"]) > 6000000:
            cwlRecord = json.loads(base64.b64decode(originalRecord["data"]))
